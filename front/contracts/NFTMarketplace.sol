...
contract NFTMarketplace is ERC721URIStorage, Latcheable {
    using Counters for Counters.Counter;
    //_tokenIds variable has the most recent minted tokenId
    Counters.Counter private _tokenIds;
    //Keeps track of the number of items sold on the marketplace
    Counters.Counter private _itemsSold;
    //ownerContract is the contract address that created the smart contract
    ...
    //Returns all the NFTs that the current user is ownerContract or seller in
    function getMyNFTs() public view returns (ListedToken[] memory) {
        ...
        for (uint256 i = 0; i < totalItemCount; i++) {
            if (
                idToListedToken[i + 1].ownerContract == msg.sender ||
    ...
    function executeSale(uint256 tokenId) public payable {
        ...
        //Actually transfer the token to the new ownerContract
        _transfer(address(this), msg.sender, tokenId);
        //approve the marketplace to sell NFTs on your behalf
        approve(address(this), tokenId);
        //Transfer the listing fee to the marketplace creator
        payable(ownerContract).transfer(listPrice);
    ...
    function updateListPrice(uint256 _listPrice) public payable {
        require(
            ...
            "Only ownerContract can update listing price"
        );
